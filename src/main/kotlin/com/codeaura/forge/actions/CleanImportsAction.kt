...
import com.codeaura.forge.engine.ImportCleaner
...
class CleanImportsAction : AnAction() {
    override fun actionPerformed(e: AnActionEvent) {
        val project  = e.project ?: return
        val editor   = e.getData(CommonDataKeys.EDITOR) ?: return
        val file     = e.getData(CommonDataKeys.VIRTUAL_FILE) ?: return
        val registry = ApplicationManager.getApplication().getService(FileConfigRegistry::class.java)
        registry.getOrCreate(file).cleanImports = true
        val result = ImportCleaner().process(editor.document.text)
